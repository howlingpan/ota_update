...
public class OtaUpdatePlugin implements FlutterPlugin, ActivityAware, EventChannel.StreamHandler,
  ...
  /**
   * Legacy plugin initialization for embedding v1. This method provides backwards
   * compatibility.
   *
  ...
  private void executeDownload() {
    try {
//      String dataDir = context.getFilesDir() + "/files/ota_update";
      String dataDir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath() ;
      Log.d(TAG, dataDir);
      // PREPARE URLS
        ...
        if (progressSink != null) {
          ...
          if (data.containsKey(ERROR)) {
            ...
            progressSink.success(
                Arrays.asList("" + OtaStatus.DOWNLOADING.ordinal(), "" + ((bytesDownloaded * 100) / bytesTotal)));
          }
        }
      }
    };
